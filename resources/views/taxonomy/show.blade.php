...
<section class="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">
    ...
    <div class="mt-7 max-w-4xl">
        <p class="text-sm font-extrabold uppercase tracking-[0.18em] text-indigo-600">{{ $node->type->value }}</p>
        <h1 class="mt-3 text-5xl font-extrabold tracking-[-0.035em]">{{ $node->name }}</h1>
        @if($node->description)
            <p class="mt-5 max-w-3xl text-lg leading-8 text-slate-600 dark:text-slate-300">{{ $node->description }}</p>
        @endif
    </div>
    ...
    @if($children->isNotEmpty())
        <div class="mt-12">
            <h2 class="text-3xl font-extrabold">Continuă pe subiect</h2>
            <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                @foreach($children as $child)
                    <a href="{{ $urlGenerator->taxonomy($child) }}" class="rounded-3xl border border-slate-200 bg-white p-6 transition hover:-translate-y-0.5 hover:shadow-lg dark:border-white/10 dark:bg-white/5">
                        <p class="text-xs font-extrabold uppercase tracking-[0.16em] text-indigo-600">{{ $child->type->value }}</p>
                        <h3 class="mt-3 text-xl font-extrabold">{{ $child->name }}</h3>
                        @if($child->description)
                            <p class="mt-3 line-clamp-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{{ $child->description }}</p>
                        @endif
                    </a>
                @endforeach
            </div>
    ...
    <div class="mt-12">
        <div class="flex items-end justify-between gap-4">
            <div>
                <p class="text-sm font-extrabold uppercase tracking-[0.16em] text-indigo-600">Teste disponibile</p>
                <h2 class="mt-2 text-3xl font-extrabold">Exersează acum</h2>
            </div>
        </div>
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            @empty
                <div class="rounded-3xl border border-dashed border-slate-300 p-8 text-slate-500 dark:border-white/15">
                    Testele pentru această secțiune sunt în curs de publicare.
                </div>
            @endforelse
        </div>
